...
| Output | `bool` |
| Direct scan config | `scan.vcs_52_high_vcs_min`, `scan.vcs_52_high_rs21_min`, `scan.vcs_52_high_dist_max`, `scan.vcs_52_high_require_trend_base` |

## Evaluation Context

- Evaluated on one latest row after `enrich_with_scan_context()`.
- Reads a precomputed VCS score, 21-day RS, distance from the 52-week high, and trend-base state.
...
| `trend_base` | `src/indicators/core.py::IndicatorCalculator.calculate` | `False` | required when `vcs_52_high_require_trend_base` is `true` |
...
| `scan.vcs_52_high_require_trend_base` | `true` | when true, requires `trend_base` |
