...
description: Use when the current OraTek implementation is the source of truth and Codex must update the SystemDocs specifications to match it. Trigger this skill when the user asks to document current behavior from code, refresh doc/SystemDocs after code or config changes, sync design docs to the implementation, or update the specification index and related specs from the live codebase. Do not use it when the request is to change implementation to satisfy an existing spec.
---
...
1. Read `doc/SystemDocs/Specifications/00_INDEX.md` to identify the active document set.
...
4. Update only the affected files under `doc/SystemDocs/` and `00_INDEX.md` when filenames or scope change.
...
- Only `doc/SystemDocs/` is in scope for documentation edits under this skill.
